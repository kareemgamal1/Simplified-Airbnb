...
void Traveler::choosePlace()
{
	...
	for (int i = 0; i < currentPlaces.size(); i++)
	{
		if (currentPlaces[i].ID == ID)
		{
			...
			cout << "day:";
			cin >> endDate.month;
			// iterate over daysofplace
			int period = calculateBookedDuration(startDate, endDate);
			currentPlaces[i].bookingcontinousperiod(period, startDate);
			for (int j = 0; j < currentPlaces[i].daysofplace.size(); j++)
				currentPlaces[i].daysofplace[j].reserved = true;
			serializePlace(currentPlaces[i]); // should put the place inside the folder reserved places
			// then remove it from the host's places?
			// or check if all its days are reserved then delete it
			reservedPlace = currentPlaces[i];
			cout << "Please pay " << generateTotalPrice(currentPlaces[i]) << " using: " << reservedPlace.paymentMethod << endl;
			return;
		}
	}
}
...
float Traveler::generateTotalPrice(Place p)
{
	int totalPrice = 0;
	if (p.discount != 0)
	{
		float discount = p.discount / 100;
		totalPrice = (p.pricePerDay - (discount * p.pricePerDay)) * p.duration; // then multiply the whole expression by the number of days (Tamer&Ahmed)
	}
	else
	{
		totalPrice = p.pricePerDay * p.duration; // duration is calculated by the number of days the user reserves.
	}
	return totalPrice;
